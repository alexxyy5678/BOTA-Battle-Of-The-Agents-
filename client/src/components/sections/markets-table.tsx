...
function arenaBattleModeBadge(mode?: ArenaBattleMode) {
  if (mode === 'challenge') {
    return {
      label: 'Challenge',
      rail: 'Onchain',
      className: 'border-emerald-400/40 bg-emerald-500/15 text-emerald-400',
    };
  }
  ...
  return {
    label: 'Sim',
    rail: 'Offchain',
    className: 'border-sky-400/40 bg-sky-500/15 text-sky-400',
  };
}

function ArenaModeBadge({ mode }: { mode?: ArenaBattleMode }) {
  const badge = arenaBattleModeBadge(mode);
  return (
    <span className={`inline-flex w-[92px] items-center justify-center gap-1 rounded border px-1.5 py-1 text-[9px] font-black uppercase leading-none ${badge.className}`}>
      <span>{badge.label}</span>
      <span className="font-mono opacity-75">{badge.rail}</span>
    </span>
  );
}
      ...
      <div className="flex-1 overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {!isLoading && markets.length > 0 && (
          <div className="hidden md:flex items-center gap-2 lg:gap-3 px-3 py-1.5 border-b border-border bg-muted/30 text-xs font-bold text-muted-foreground uppercase tracking-wide">
            ...
            <span className="hidden lg:block w-[92px] shrink-0 text-center">{mode === 'battles' ? 'Mode' : 'Trend'}</span>
          ...
          : markets.map((market, index) => (
              ...
              >
                ...
                <div className="hidden lg:block shrink-0">
                  {market.rowType === 'battle' ? (
                    <ArenaModeBadge mode={market.arenaPreview?.mode} />
                  ) : (
          ...
          <div className="mx-3 my-3 bg-primary/10 border border-primary/30 rounded-lg px-4 py-3">
            <div className="flex items-center justify-between flex-wrap gap-3">
              <div>
                ...
                <div className="text-xs text-muted-foreground">Now pulling from live onchain and BOTA social markets.</div>
              </div>
