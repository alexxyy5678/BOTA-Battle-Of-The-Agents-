...
export const NotificationFeed: React.FC<NotificationFeedProps> = ({ maxDisplay = 5 }) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const data = await apiRequest('GET', '/api/notifications?limit=20');
        const notificationRows = Array.isArray(data) ? data : Array.isArray(data?.data) ? data.data : [];
        setNotifications(notificationRows.map((item: any) => normalizeNotification(item)));

        // Get unread count
        const countData = await apiRequest('GET', '/api/notifications/unread-count');
        setUnreadCount(Number(countData.unreadCount ?? countData.count ?? 0));
      } catch (error) {
