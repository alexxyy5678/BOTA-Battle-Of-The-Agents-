...
function normalizeNotificationsResponse(response: unknown): any[] {
...
export function useNotifications() {
  ...
  const { data: notifications = [], isLoading } = useQuery({
    ...
    queryFn: async () => normalizeNotificationsResponse(await apiRequest('GET', '/api/notifications')),
    enabled: !!user,
    refetchInterval: 30000, // Refetch every 30 seconds
  });

  const markAsReadMutation = useMutation({
    mutationFn: (id: string | number) => apiRequest('PATCH', `/api/notifications/${id}/read`),
    onSuccess: () => {
