...
import {
  executeBantCreditRewardClaimTx,
...
type OnchainBantCreditClaimsResponse = {
...
export default function WalletPage() {
  ...
  const { data: onchainBantCreditClaims } = useQuery<OnchainBantCreditClaimsResponse>({
    queryKey: ["/api/bantahbro/onchain/bantcredits/claims", connectedWalletAddress || "none"],
    queryFn: async () => await apiRequest("GET", "/api/bantahbro/onchain/bantcredits/claims"),
    enabled: Boolean(isOnchainBuild && connectedWalletAddress),
    retry: false,
    staleTime: 1000 * 20,
  });

  const claimableOnchainBantCredits =
    onchainBantCreditClaims?.claims?.filter((claim) => claim.status === "claimable") || [];
  const nextOnchainBantCreditClaim = claimableOnchainBantCredits[0] || null;

  const onchainBantCreditClaimMutation = useMutation({
    mutationFn: async (claim: OnchainBantCreditClaim) => {
      if (!onchainConfig) {
        throw new Error("Onchain config is still loading.");
      }
      const tx = await executeBantCreditRewardClaimTx({
      ...
      await apiRequest(
        "POST",
        `/api/bantahbro/onchain/bantcredits/claims/${claim.id}/mark-claimed`,
        { txHash: tx.claimTxHash },
      );
      return tx;
    },
    onSuccess: () => {
      toast({
        title: "BantCredits claimed",
        description: "Your onchain reward claim was confirmed.",
      });
      queryClient.invalidateQueries({
        queryKey: ["/api/bantahbro/onchain/bantcredits/claims"],
      });
    },
    onError: (error: Error) => {
      toast({
              ...
              <div className="flex items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-2">
                  <div className="w-9 h-9 rounded-xl bg-lime-100 dark:bg-lime-900/30 flex items-center justify-center">
                    <ShieldCheck className="w-4 h-4 text-lime-700 dark:text-lime-300" />
                  </div>
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      {selectedChain?.bantCreditsAddress
                        ? selectedChain.name
                        : "Awaiting contract config"}
                    </p>
                  </div>
                </div>
                <Badge className="border-0 bg-lime-100 text-lime-800 dark:bg-lime-900/40 dark:text-lime-200 text-[10px]">
                  {onchainBantCreditClaims?.claimableBantCredits || 0} claimable
                </Badge>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-3 items-center">
                <div className="min-w-0">
                  {onchainBantCreditClaims?.claims?.length ? (
                    <div className="space-y-2">
                      {onchainBantCreditClaims.claims.slice(0, 3).map((claim) => (
                        <div
                          key={claim.id}
                          className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 dark:bg-slate-900 px-3 py-2"
                        >
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-slate-900 dark:text-slate-100 truncate">
                              {claim.role.replace(/_/g, " ")}
                            </p>
                            <p className="text-[11px] text-slate-500 dark:text-slate-400 truncate">
                              {claim.chainName || `Chain ${claim.chainId}`} · {claim.status}
                            </p>
                          </div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">
                      No simulated battle claim batch yet.
                    </p>
                  )}
                </div>
                ...
                <Button
                  onClick={() =>
                    nextOnchainBantCreditClaim &&
                    onchainBantCreditClaimMutation.mutate(nextOnchainBantCreditClaim)
                  }
                  disabled={
                    !nextOnchainBantCreditClaim ||
                    !selectedChain?.bantCreditRewardsAddress ||
                    onchainBantCreditClaimMutation.isPending
                  }
                  className="h-10 rounded-xl text-black font-semibold border-0"
                  style={{ backgroundColor: "#ccff00" }}
                >
                  {onchainBantCreditClaimMutation.isPending ? "Claiming..." : "Claim"}
                </Button>
              </div>
            </div>
