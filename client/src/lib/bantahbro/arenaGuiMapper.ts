...
function sideLeaderboardRank(side: AgentBattleSide) {
  const candidates = [side.leaderboardRank, side.rank, side.profileRank, side.level];
  const directRank = candidates.find((candidate) => Number.isFinite(Number(candidate)) && Number(candidate) > 0);
  if (directRank !== undefined) return Math.max(1, Math.round(Number(directRank)));
  return 0;
}
...
function sideRankDelta(side: AgentBattleSide, nextRank: number) {
  const previousRank = [side.previousRank, side.oldRank, side.priorRank]
    .map(positiveRank)
    .find((rank): rank is number => rank !== null);
  if (previousRank !== undefined) return previousRank - nextRank;

  const directDelta = [side.rankDelta, side.rankChange, side.rankMovement]
...
function mapSideState(
  ...
  const leaderboardRank = sideLeaderboardRank(side);
  const rankDelta = sideRankDelta(side, leaderboardRank);
  return {
    id: side.id,
    index,
    ...
    previousRank: rankDelta !== 0 ? Math.max(1, leaderboardRank + rankDelta) : null,
    rankDelta,
    health: confidence,
    status: side.status,
...
export function mapBattleToArenaGuiState(battle: AgentBattle): ArenaGuiState {
  ...
  return {
    ...
    spectators: Math.max(0, Math.round(Number(battle.spectators || 0))),
    bantCreditsEarned: Math.max(0, Math.round(Number(battle.bantCreditsEarned || 0))),
    spectatorBantCredits: Math.max(0, Math.round(Number(battle.spectatorBantCredits || 0))),
    durationSeconds,
