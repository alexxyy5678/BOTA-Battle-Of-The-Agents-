...
const bantCreditRewardsAbi = parseAbi([
  "function claim(bytes32 batchId, address account, uint256 amount, bytes32 role, bytes32 matchId, bytes32[] proof)",
]);

const BASE_ATTRIBUTION_CHAIN_IDS = new Set<number>([8453, 84532]);
const BASE_BUILDER_CODE = String(
  (import.meta as any)?.env?.VITE_BASE_BUILDER_CODE || "bc_aujwsbcz",
).trim();
...
export async function executeBantCreditRewardClaimTx(params: {
  wallets: PrivyWallet[];
  preferredWalletAddress?: string | null;
  onchainConfig: OnchainRuntimeConfig;
  chainId: number;
  ...
  proof: `0x${string}`[];
}): Promise<{ claimTxHash: `0x${string}`; walletAddress: `0x${string}` }> {
  const chainConfig = resolveChainConfig(params.onchainConfig, Number(params.chainId));
  if (!chainConfig) {
    throw new Error("Unsupported chain for BantCredits claim.");
  }

  const rewardsAddress = normalizeAddress(chainConfig.bantCreditRewardsAddress);
  if (!rewardsAddress) {
    throw new Error(`BantCreditRewards is not configured for ${chainConfig.name}.`);
  }
  ...
  if (sender.toLowerCase() !== account.toLowerCase()) {
    throw new Error("Connect the wallet that owns this BantCredits claim.");
  }
  ...
  const data = encodeFunctionData({
    ...
    args: [
      ...
      amount,
      params.roleBytes32,
      params.matchIdBytes32,
      params.proof,
    ],
  });

  const claimTxHash = await sendTransactionAndWait({
    provider,
    from: sender,
    to: rewardsAddress,
    data,
  ...
  return { claimTxHash, walletAddress: sender };
}
