window.addEventListener("DOMContentLoaded", async () => {
  const assetVersion = "engine-20260610-perf-audio01";
  const params = new URLSearchParams(window.location.search);
  const isArenaEmbed = params.get("battle-section") === "1";
  const fighterPaths = [
    `data/fighters/bantah-mascot-green.json?v=${assetVersion}`,
    `data/fighters/bantah-mascot-orange.json?v=${assetVersion}`,
  ];
  const root = document.querySelector(".bantah-fighting-game") || document;
  root.classList?.add?.("is-mascot-arena");
