...
function arenaStageMessage(input: {
  ...
  if (input.stage === "starting_soon") {
    return {
      ...
      title: "Arena starts in 5 minutes",
      message: `${input.fighterName} faces ${input.opponentName} in about 5 minutes.`,
      priority: 4,
      fomoLevel: "urgent",
    };
  }
...
async function notifyUpcomingArenaQueue(battles: BantahBroAgentBattle[]) {
  ...
  for (const battle of battles) {
    ...
    if (Number.isFinite(startsInMs) && startsInMs > 0 && startsInMs <= 5 * 60 * 1000 + 30_000) {
